...
UserCommand::UserCommand() : operation { CMD_NONE }, params_data { nullptr } {
	char* p_data = new char[RECORD_SIZE];
	memset(p_data, 0, RECORD_SIZE * sizeof(char));
	params_data = boost::shared_ptr<char>(p_data);
}
...
UserCommand* UserCommand::read_from_buffer(struct evbuffer* buff) {
	...
	if (operation_string == "get") {
		...
		strncpy(p_data + KEY_SIZE + 1, value_ptr, value_str.size() + 1);
	}
	...
	strncpy(p_data, key_ptr, key_str.size() + 1);
	cmd->tokens.clear();
...
const char* UserCommand::get_key_param() const {
	return params_data.get();
}

char* UserCommand::get_value_param() const {
	char* p_data = params_data.get();
	return &p_data[KEY_SIZE + 1];
}
